...
// com a classe 'tech';
input.addEventListener('keyup',changeTextClassTech);
function changeTextClassTech (event) {
...
myWebpage.addEventListener('dblclick',linkSite);
function linkSite (event) {
    window.open('https://github.com/fkamag', 'blank');
}

// 5. Crie uma função que, ao passar o mouse sobre 'Meu top 3 do Spotrybefy', altere
// a cor do mesmo;
myWebpage.addEventListener('mouseover',changeTextColor);
// myWebpage.addEventListener('mouseout',changeTextColor);
myWebpage.addEventListener('mouseout', function(event) {
    event.target.style.color = 'unset';
  }); /* retirado do gabarito */

function changeTextColor(event) {
    myWebpage.style.color = gerar_cor();
}
